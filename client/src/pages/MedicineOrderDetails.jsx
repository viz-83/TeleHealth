    ...
    return (
        <div className="min-h-screen bg-background-light p-4 md:p-8">
            <div className="max-w-3xl mx-auto">
                {/* Header */}
                <div className="bg-white dark:bg-surface rounded-2xl p-6 shadow-sm mb-6 border border-gray-100 dark:border-gray-700">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <h1 className="text-2xl font-bold text-text-primary">Order #{order._id.slice(-6)}</h1>
                            <p className="text-text-secondary text-sm">Placed on {format(new Date(order.createdAt), 'PPP')}</p>
                        </div>
                                    ...
                                    return (
                                        <div key={step.status} className="flex flex-col items-center">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 transition-colors ${isCompleted
                                                ? 'bg-teal-600 border-teal-100 dark:border-teal-900/50 text-white'
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Delivery Address */}
                    <div className="bg-white dark:bg-surface rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 h-fit">
                        <h2 className="font-bold text-text-primary mb-4 flex items-center gap-2">
                        ...
                        <div className="text-text-secondary leading-relaxed">
                            ...
                            <p className="font-medium text-text-primary">PIN: {order.deliveryAddress.pincode}</p>
                        </div>
                    </div>
                </div>
            </div>
